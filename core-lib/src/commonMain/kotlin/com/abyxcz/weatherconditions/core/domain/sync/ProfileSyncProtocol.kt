...
object ProfileSyncProtocol {
    ...
    /**
     * Derive the 32-byte pre-shared key the BLE library expects from a human pairing code.
     * Both sides run this identical derivation, so the same code yields the same PSK and the
     * authenticated handshake succeeds. Blank codes should be treated as "no PSK" by callers.
     */
    fun derivePsk(pairingCode: String): ByteArray = pairingCode.encodeUtf8().sha256().toByteArray()
